...
figure_dir<- paste0(working_dir, "/figures/FACs/")
if(!dir.exists(figure_dir)){dir.create(figure_dir)}

analysis_dir<- paste0(working_dir, "/analysis/FACs/")
if(!dir.exists(analysis_dir)){dir.create(analysis_dir)}
```

## Read in FACs data
```{r}
facs_df<- read.csv("FACsPercentages.csv")
...
facs_df$Subtype<- gsub("hemiBCOR", "BCOR", 
                       gsub("hemiG34R","Hemi_H3G34R",
                            gsub("midlineHistoneWT", "Midline_HistoneWT",
                                 gsub("midlineK27M", "Midline_H3K27M", 
                                      gsub("hemiHistoneWT", "Hemispheric_HistoneWT", facs_df$Subtype)))))
...
ggplot(facs_melt, aes(x=reorder(Sample, Order), y=ImmuneCellProportion, fill=CellType))+
  ...
  xlab("")+
  scale_fill_manual(values=c(TCell="red", Myeloid="blue"))
...
ggplot(facs_melt, aes(x=reorder(Sample, Order), y=ImmuneCellProportion, fill=CellType))+
  ...
  scale_fill_manual(values=c(TCell="red", Myeloid="blue", TumorCells="olivedrab"))
