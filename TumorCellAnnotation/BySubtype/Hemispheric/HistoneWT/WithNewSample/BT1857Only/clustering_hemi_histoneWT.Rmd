...
```{r load-data}
load(paste0(project_dir, "SplitBySubtype/WithNewSamples/analysis/hemiHistoneWT_cm_list.Robj"))
samples = readRDS(paste0(project_dir, "SplitBySubtype/WithNewSamples/analysis/hemiHistoneWT_samples.rds"))
tumor_stat = readRDS(paste0(project_dir, "SplitBySubtype/WithNewSamples/analysis/hemiHistoneWT_call_tumor.rds"))


## Subset to BT1857
samples<- samples[samples=="BT1857"]
tumor_stat<- tumor_stat[names(tumor_stat) %in% names(samples)]
cm_list<-lapply(hemiHistoneWT_cm_list, function(x){x[,colnames(x) %in% names(samples)]})
...
ggsave("UMAP_clustering_Resolution0.8.jpg", path=seurat_fig_folder, width=4, height=4)

## By samples
DimPlot(object = seurat_obj, reduction="umap", group.by = "sample", label = FALSE, 
        pt.size = 2, label.size = 5) + NoAxes() 
ggsave("UMAP_sample.jpg", path=seurat_fig_folder, width=5, height=4)

## By manual annotation
DimPlot(object = seurat_obj, reduction="umap", group.by = "ManualAnnot", label = FALSE, 
        pt.size = 2, label.size = 5) + NoAxes() 
ggsave("UMAP_ManualAnnot.jpg", path=seurat_fig_folder, width=5, height=4)
...
FeaturePlot(seurat_obj,
            ...
            cols = c("lightgrey", "red"),
            ncol = 2)
ggsave("PAX6_markers.jpg", path=seurat_fig_folder, width=9, height=4.5)
```
...
ggsave(paste0("GO_OVA_AllClusters_top20.jpg"), path=seurat_fig_folder, width=18, height=12)
...
```{r}
hemi_genesets<-readRDS(paste0(project_dir, "../../PublishedDatasets/FilbinPublishedDatasets/Metaprograms/",
                                 "hemisphericPreviousPrograms_2019_OC.rds"))
...
## Manually annotate cells
## Based primarily on marker genes and Neftel programs
```{r}
clusters<- c("0", "1", "2")
names(clusters)<-c("NPC-like", "AC-like", "Cycling")
