...
nmf_meta_programs[["NMF2"]]<- colnames(nmf_factor_cor)[7:10]
nmf_meta_programs[["NMF3"]]<- colnames(nmf_factor_cor)[13:16]
nmf_meta_programs[["NMF4"]]<- colnames(nmf_factor_cor)[17:18]
nmf_meta_programs[["NMF5"]]<- colnames(nmf_factor_cor)[19:20]
...
tmp2 = sapply(nmf_marker_genes_final, function(x){
    sapply(hemi_genesets, function(y){
        fisher_test(x, y, unlist(union(hemi_genesets, nmf_marker_genes_final)))
    })
})
tmp2 = data.frame(tmp2)
tmp4<-apply(tmp2, 2, function(x) {p.adjust(x)})

write.table(tmp4, paste0(nmf_analysis_folder,
...
allOE_genes <- as.character(gene_symbol_to_ensembl_id(rownames(cm_list$raw_data),
...
ggsave(paste0("GO_OVA_AllMetaprograms_top20.jpg"), path=nmf_fig_folder, width=18, height=20)
save(go_result, file=paste0(nmf_analysis_folder,  "go_result_AllMetaprograms.Robj"))
