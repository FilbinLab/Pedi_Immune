...
working_dir<- paste0(base_dir, "Dropbox (Partners HealthCare)/FilbinLab/data_analysis/Projects/Immune/Annotation/BySubtype/Hemispheric/HistoneMut/")
...
```{r load-data}
load("../../analysis/hemiHistonemut_cm_list.Robj")
samples = readRDS("../../analysis/hemiHistonemut_samples.rds")
tumor_stat = readRDS("../../analysis/inferCNV/seurat/hemiHistonemut_call_tumor.rds")

cm_list<-hemiHistonemut_cm_list
...
ggsave("UMAP_clustering_Resolution0.6_theta2.jpg", path=seurat_fig_folder, width=8, height=8)

## By samples
DimPlot(object = seurat_obj, reduction="umap", group.by = "sample", label = FALSE, 
        pt.size = 2, label.size = 5) + NoAxes() 
ggsave("UMAP_sample_theta2.jpg", path=seurat_fig_folder, width=10, height=8)
...
all_markers_filtered = all_markers[all_markers$p_val_adj < 0.05 & all_markers$avg_logFC > .5, ]
...
```{r}
VlnPlot(seurat_obj, 
       c("CLEC2D", "WNT5A", "PVR", "CDH1", "SPP1"), group.by="TopSigPrevious", 
        pt.size = 0, sort = TRUE) 
...
# Score cells based on Ilon's G34RV programs
```{r}
g34rv_programs<- read.csv("analysis/FromIlon_G34RV_Markers_FreshFrozen.csv")
gene_list<-list(OC_like=g34rv_programs$fresh.OC.like,
                AC_like=g34rv_programs$fresh.AC.like,
                cycling=g34rv_programs$fresh.cycling,
                GIPC_like=g34rv_programs$fresh.GIPC.like)
...
#ggsave(paste0(seurat_fig_folder, "ScoredBasedIlonsG34RVPrograms.png"))
...
ggsave("UMAP_clustering_CellChatAnnot.jpg", path=seurat_fig_folder, width=7, height=7)
...
ggsave(paste0(seurat_fig_folder, "CellBreakdown.png"), width=6, height=3)
```