...
working_dir<- paste0(base_dir, "Dropbox (Partners HealthCare)/FilbinLab/data_analysis/Projects/Immune",
...
## Create output files
inferCNV_analysis_folder = "analysis/inferCNV_Replicate/HC/"
inferCNV_fig_folder = "figures/inferCNV_Replicate/HC/"
...
inferCNV_input_folder<- "analysis/inferCNV_Replicate/"
```
...
## Compare cnv_res to analysis without 2 new samples (BT1873/BT1857)
```{r}
## Load in old results
original_cnv_res<- readRDS("../analysis/inferCNV/HC/call_cnv.rds")
 
## remove new samples from current results
new_cnv_res<- call_cnv_res[grep(paste(c("BT1873", "BT1857"),collapse="|"), 
                                names(call_cnv_res),invert=TRUE)]
sum(names(new_cnv_res)==names(original_cnv_res))

## Make df with old/new cnv stats
merge_cnv_res<- as.data.frame(original_cnv_res)
merge_cnv_res$new_cnv_res<- plyr::mapvalues(rownames(merge_cnv_res),
                                            from=names(new_cnv_res),
                                            to=new_cnv_res)
sum(merge_cnv_res$original_cnv_res==merge_cnv_res$new_cnv_res) 

## If threshold set at 5: 4,440 out of 4,678 (95%)
## If threshold set at 9: 4,602 out of 4,678 (98%) (anything 5-9: 4,440. anything 9-18)
## Threshold set at 9 for downstream

## Which cells/samples do not match? Save to view in inferCNV UMAP
merge_cnv_res$sample<- plyr::mapvalues(rownames(merge_cnv_res),
                                       from=names(samples),
                                       to=samples, warn_missing = FALSE)

merge_cnv_res$Match<- ifelse(merge_cnv_res$original_cnv_res==merge_cnv_res$new_cnv_res,
                             yes="Match",
                             no="noMatch")
table(merge_cnv_res$Match, merge_cnv_res$sample)

## non-matching cells spread out over multiple samples: BT1516/1678/1733/1745/CPD/MUV63/66/68/79/83
## Max mismatch: 19 cells for BT1733
## Only 3 samples with more than 10 cells mismatch (BT1733, BT1745, MUV083- 3 different subtypes)
cnv_match_WithWithoutNewSamples<- merge_cnv_res
save(cnv_match_WithWithoutNewSamples, file="analysis/cnv_match_withWithoutNewSamples.Robj")
```
