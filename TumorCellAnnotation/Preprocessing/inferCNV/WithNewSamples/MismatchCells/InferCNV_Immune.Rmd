...
```{r read-data}
input_dir<- "../../../QC/WithNewSamples/analysis/"
...
## Subset to cells that mismatch between original (no new samples)/new analysis (with new)
## Only cells that changed from malig->normal/noise or normal/noise->malig
load("../analysis/inferCNV/seurat/CellsThatMismatchBetweenOriginalNewAnalysis.Robj")
samples<-samples[names(samples) %in% mismatchCells$Row.names]
cm_list<-lapply(cm_list, function(x){x[,colnames(x) %in% mismatchCells$Row.names]})
any(colnames(cm_list$raw_data) != names(samples))
