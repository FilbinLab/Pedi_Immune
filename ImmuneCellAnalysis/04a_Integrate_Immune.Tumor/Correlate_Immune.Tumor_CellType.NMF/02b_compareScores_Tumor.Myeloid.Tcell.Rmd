...
figure_dir<- paste0(figure_dir_tmp, cohort, "/byScores/");analysis_dir<- paste0(analysis_dir_tmp, cohort, "/")
if(!dir.exists(figure_dir)){dir.create(figure_dir)}; if(!dir.exists(analysis_dir)){dir.create(analysis_dir)}


## Mean scores by sample
score_list<- readRDS(paste0(analysis_dir, "program_scores_myeloid.cd4.cd8.tumor.Rds"))
...
ggplot(pearson_df, aes(x=p1, y=p2, fill=PearsonCorrelation))+
  ...
  xlab("") + ylab("")
ggsave(paste0(figure_dir, "Heatmap_pearsonCorr_lmForSig_cd4.cd8.myeloid.tumor.Scores.png"), width=7.5, height=5)
```
...
## Heatmap with correlation for all
```{r}
## Add major cell type to program name
all_scores<- lapply(names(score_list), function(x){
  tmp<- score_list[[x]]
  tmp$Program<- paste0(x, "_", tmp$Program)
  tmp$CellType<-x
  return(tmp)
})
names(all_scores)<- names(score_list)

## Reformat into single df
scores_df<- do.call("rbind", all_scores)

test<- split(scores_df, f=scores_df$Sample)
