...
Note that currently (10/26/22), myeloid/tcells run together for simplicity, but in the "tcells" directory
```{r read-data}
...
myeloid_data_dir<- "../../../02b_Myeloid/03_CompareToAdult/analysis/Merge_AssessIntegration/pedOnly_nomuv63/adult_withMUV.FALSE/"
...
```{r load-seurat-obj}
cd8_seurat<- readRDS(paste0(tcell_data_dir, "adult_seurat_cd8_harmony.Rds"))
cd4_seurat<- readRDS(paste0(tcell_data_dir, "adult_seurat_cd4_harmony.Rds"))
myeloid_seurat<- readRDS(paste0(myeloid_data_dir, "myeloid_seurat_Ped.AdultGBM.IDHMut_merged_harmony.Rds"))
myeloid_seurat<- subset(myeloid_seurat, Age=="Adult")
myeloid_seurat$Final_Annot<- myeloid_seurat$MaxProgram_PA_mergedGenesets
