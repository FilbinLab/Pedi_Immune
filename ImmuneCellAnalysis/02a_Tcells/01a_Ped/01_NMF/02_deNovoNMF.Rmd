...
## Approach:
Run NMF on O2, then finish processing here-identify gene lists, map to seurat, correlate with ped-derived CD4/CD8 programs to help annotate.
...
analysis_dir_tmp<- paste0(working_dir, "/analysis/deNovoNMF/")
figure_dir_tmp<- paste0(working_dir, "/figures/deNovoNMF/")
...
## Read in ped tcell seurat object. Merged in Merge_AssessIntegration.Rmd
```{r}
cohort<- "pedOnly_nomuv63"

## Current seurat object
tcell_seurat<- readRDS(paste0("../../../01_Preprocessing/02_Counts_Preprocess/analysis/detailed_annotation/",
                              cohort,"/tcell_seurat.Rds"))
...
cd8_markers<- read.csv(paste0("ARCHIVE/CD8/analysis/", cohort, "/top_DE_genes_annotated_NMF_rank4.csv"))
cd4_markers<- read.csv(paste0("ARCHIVE/CD4/analysis/", cohort, "/top_DE_genes_annotated_NMF_rank4.csv"))
...
estim.r <- nmf(x, rank = rank, method = "brunet", nrun = 10, seed = 123456, .options = "tvP4")
...
png(paste0(figure_dir, "_NMF_plot_rank8-10.png"), width = 2000, height = 1000, res = 100)
plot(estim.r)
dev.off()
...
saveRDS(tcell_seurat, file=paste0("../../Preprocessing/Counts_Preprocess/analysis/detailed_annotation/",
                              cohort,"/tcell_seurat.Rds"))
```
