...
## Plot heatmaps of top marker gene expression- pseudobulk by CD4/CD8
```{r}
rank<- "8"
sharedBy<- "union"
add_lines<- TRUE
...
## Pseudobulk
tcell_seurat$PB_variable<- paste0(tcell_seurat$detailed_annot_byNMFrank8_mergedByunion, 
...
ggsave(paste0(figure_dir, "Heatmap_TopNMFgenes_annot_rank", rank, ".png"), width=17, height=11)
...
df<-tcell_seurat_use@meta.data[,c("sampleid", "NiceSubtype", program_name)]
...
nCells<- as.data.frame(table(tcell_seurat_use$NiceSubtype))
df$NCells<- plyr::mapvalues(tcell_seurat_use$NiceSubtype, nCells$Var1, nCells$Freq)
...
all_plots<- list()
for(i in as.character(unique(tcell_seurat_use@meta.data[[program_name]]))){
  print(i)
...
DotPlot(tcell_seurat, features=c("naive", "cytotoxic", "predysfunctional", "dysfunctional",
                               "ENTPD1", "ITGAE", "TNFRSF9", "CCL4L1", "PDCD1", "CTLA4", "LAG3", "GZMK", "GNLY"),
        group.by = "tmp")+
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.text = element_text(face="bold", color="black"))+
  ylab("") + xlab("")
ggsave(paste0(figure_dir, "DotPlot_Nature2020.GOI_byProgram.png"), width=8, height=5)

## Inhibitory GOI
DotPlot(tcell_seurat, features=c( "ITGAE", "TNFRSF9",  "PDCD1", "CTLA4", "LAG3", "TIGIT", "KLRB1", "HLA-DRA", "PTGER2"),
        group.by = "tmp")+
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.text = element_text(face="bold", color="black"))+
  ylab("") + xlab("")
...
df<- tcell_seurat@meta.data[,c("tmp", "naive", "cytotoxic", "predysfunctional", "dysfunctional")]
