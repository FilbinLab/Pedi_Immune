...
DotPlot(cd8_seurat, features=c(names(all_markers),
                               "ENTPD1", "ITGAE", "TNFRSF9", "CCL4L1", "PDCD1", "CTLA4", "LAG3", "GZMK", "GNLY"),
        group.by = "tmp")+
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.text = element_text(face="bold", color="black"))+
  ylab("") + xlab("")
ggsave(paste0(figure_dir, "CD8_DotPlot_Nature2020.GOI_byProgram.png"), width=8, height=5)
...
DotPlot(cd8_seurat, features=c( "ITGAE", "TNFRSF9",  "PDCD1", "CTLA4", "LAG3", "TIGIT", "KLRB1", "HLA-DRA"),
  ...
  theme(axis.text.x = element_text(angle=45, hjust=1),
        axis.text = element_text(face="bold", color="black"))+
  ylab("") + xlab("")
ggsave(paste0(figure_dir, "CD8_DotPlot_Inhibitory.GOI_byProgram.png"), width=8, height=5)
```
...
df<- cd8_seurat@meta.data[,c("tmp", names(all_markers))]

## option 1: cyto/naive on 1 scale, predys/dys on the other scale
df$scale2<- df$cytotoxic-df$naive
