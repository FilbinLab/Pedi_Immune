...
## Read in seurat list, harmony-integrated for each program separately, split by cd4/cd8 (below)
seurat_list<- readRDS(paste0(analysis_dir, "seurat_list_splitByNMF.CD4.CD8.Rds"))
...
## Dotplot of Nature2020 scores + GOI, separately for CD4/CD8
```{r}
DotPlot(cd8_seurat, features=c("ENTPD1", "ITGAE", "TNFRSF9", "CCL4L1", "PDCD1", "CTLA4", "LAG3", "GZMK", "GNLY",
                               names(all_markers)),
        group.by = "tmp")+
  theme(axis.text.x = element_text(angle=45, hjust=1))

## remove cycling genes from seurat object, rerun clustering- where does Inhibitory.Cycling fall?
```
...
```{r}
program_name<-paste0("NMF_rank", rank_use, "_annot_mergedBy", sharedBy, "_reAnnot")
subset_to<- c("Cytotoxic", "Predysfunctional", "Inhibitory.Cycling", "Inhibitory")
remove_cycling<- FALSE
analysis_name<- "Cyto.Predys"
...
FeaturePlot(cd8_cyto.inh, features=c("ENTPD1", "ITGAE", "TNFRSF9", "CCL4L1", "PDCD1", "CTLA4", "LAG3", "GZMK", "GNLY", "KLRB1", "TIGIT"),
            reduction="pca")
ggsave(paste0(figure_dir, "CD8_",analysis_name, "_PCA_dysfunction.cyto.markers.png"), width=12, height=9)
