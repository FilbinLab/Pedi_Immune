...
for(analysis in analysis_names){
  ...
  ## 2. Plot difference in expression vs difference in CV 
    ...
    outliers_all<- cnc_res[cnc_res$Expr_background_zscore>1.5 & cnc_res$CV_background_zscore < -1.2,]
    ...
    ggplot(cnc_res, aes(x=CV_background_zscore, y=Expr_background_zscore, color=GOI, alpha=GOI))+
      geom_point()+
      #scale_color_manual(values=c(yes="red", no="grey71"))+
      scale_color_manual(values=c(GOI="red",outlier="black", no="grey71"))+
      geom_text(aes(label=label), size=2, nudge_x = 0.3, nudge_y=-0.1)+
      scale_alpha_manual(values=c(GOI=1, outlier=1, no=0.2))+
      theme_classic()+
      xlim(-6.5,6.5)+
      ggtitle(analysis)
    
    ggsave(paste0(figure_dir, analysis, "_ExprCVZscores_GOI.outliers.png"), width=6, height=4.5)
