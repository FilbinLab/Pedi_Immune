...
## PGE enzymes in Myeloid
  DotPlot(ped_myeloid_seurat, features=c("PTGES2", "PTGS1", "PTGS2"), group.by = "sampleid") + DotPlot(ped_myeloid_seurat, features=c("PTGES2", "PTGS1", "PTGS2"), group.by = myeloid_program_name)
...
## PTGER4 in CD8
  ...
  ## With adult, by subtype + age (over 25, 4-25, under 4)
    under4<- c("BT1478", "BT1857", "BT1733", "MUV078")
    df<- pedAdult_cd8_seurat@meta.data
    df$PTGER2<- pedAdult_cd8_seurat@assays$RNA@data["PTGER2",]
    df$Age<- plyr::mapvalues(rownames(df), colnames(pedAdult_cd8_seurat), pedAdult_cd8_seurat$Age)
    df$Age<- ifelse(df$sampleid %in% under4, "Under4",df$Age)
    df$Age<- gsub("Ped", "Ped_4-25", df$Age)
    
    ## Call SPP1/CD44 as expressed or not
    df<- df[,c("sampleid", "PTGER2", "Age", "clonal")]
    df$PTGER2_expressed<- ifelse(df$PTGER2>2, "Expressed", "NotExpressed")
    df$clonal<- gsub("not clonal", "nonclonal", df$clonal)
    
    
    ## Determine mean expression/mean percent expressed for each age/program
    df$UniqueID<- paste0(df$Age, "__", df$clonal)
    df_summary<- df %>% group_by(UniqueID) %>% summarise(NormalizedExpression=mean(PTGER2),
                                                         PercentExpressed=sum(PTGER2_expressed =="Expressed")/n()) %>%
      as.data.frame()
    
    ## Split uniqueID back into age/program
    df_summary$Age<- unlist(lapply(strsplit(df_summary$UniqueID, split="__"),function(x){x[1]}))
    df_summary$Clonality<- unlist(lapply(strsplit(df_summary$UniqueID, split="__"),function(x){x[2]}))
    
    ## Plot
    ggplot(df_summary, aes(x=Clonality, y=Age, color=NormalizedExpression, size=PercentExpressed))+
      geom_point()+
      theme_bw()+
      ...
      xlab("")+ylab("")
    ggsave(paste0(figure_dir_use, "PTGER2_ByAgeGroup.clonality.png"), width=4, height=4)
...
DotPlot(ped_cd_seurat, group.by = tcell_program_name, features="SELPLG", scale=FALSE)
...
ggsave(paste0(figure_dir_use, "DotPlot_SELPLG_CD8_byAge.Clonal.png"), width=5, height=3)
