...
## Read in tpm, qc
```{r}
cm<- read.csv("../data/cm_tpm_02_rsem.csv", row.names = "gene_id")
qc<- read.csv("../data/qc_02_rsem.csv")

## remove "undetermned"
cm<-cm[,!grepl("Undetermined",  colnames(cm)) ]
qc<- qc[!grepl("Undetermined", qc$name),]
```
...
saveRDS(seurat_obj, file=paste0(analysis_dir, "immune_seurat.Rds"))
```

## Low quality (mitochondrial) cluster- remove
```{r}
seurat_obj<- readRDS(paste0(analysis_dir, "immune_seurat.Rds"))
