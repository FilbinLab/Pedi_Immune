...
Annotate at broad level (myeloid, tcells) based on cluster-wise expression of marker genes. Identify and remove any non-immune normal cells
...
```{r}
immune_seurat<- readRDS("analysis/seurat_obj.Rds")
...
```{r}
## reduce resolution to merge myeloid clusters
immune_seurat<- FindClusters(immune_seurat, resolution = 0.3)

## Vln plot of markers
VlnPlot(immune_seurat,  features=c("CD3G", "CSF1R")) + 
...
immune_seurat$broad_annot<- ifelse(immune_seurat$seurat_clusters == 2, "Tcell", "Myeloid")
DimPlot(immune_seurat, group.by = "broad_annot")+scale_color_manual(values=immune_colors[names(immune_colors) %in% immune_seurat$broad_annot])
