...
## Read in scores
all_scores<- readRDS(paste0(data_dir, "cd8_program_scores.Rds"))
...
top_markers<-all_markers[! all_markers$cluster %in% c("Treg", "Activated_EM"),]

## Pseudobulk
cd8_seurat$PB_variable<- paste0(cd8_seurat$sample, "_", 
...
ggsave(paste0(figure_dir, "CD8_Heatmap_TopNMFgenes_annot_rank", rank_use,".pdf"), device="pdf",width=17, height=11)
...
my_barchart_programProp(cd8_seurat, coloring_variable = "tmp", colors=program_colors)
...
## Expression of GOI
```{r}
VlnPlot(cd8_seurat, group.by = "NMF_rank9_annot", features="SELPLG", sort=TRUE)+
  NoLegend()+
  scale_fill_manual(values=program_colors)
ggsave(paste0(figure_dir, "Vln_SELPLG_byProgram.png"), width=5, height=4.5)

DotPlot(cd8_seurat, group.by = "NMF_rank9_annot", features="SELPLG")+
  scale_color_gradient2(low="blue", mid="white", high="red")+
  theme(axis.text = element_text(face="bold", color="black"))+
  xlab("")+ylab("")
ggsave(paste0(figure_dir, "DotPlot_SELPLG_byProgram.pdf"), device="pdf", width=5, height=4.5)
```
