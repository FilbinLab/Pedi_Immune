...
Note- seurat objects are for 10x pd1/untreated together- these have the most updated annotations. pd1 treated removed
```{r}
tenx_data_dir<- paste0("../../../PD1_Treat/comparisonToUntreated/analysis/project_programs/")
full_cohort_dir<- "../../"
...
names(tenx_seurat_list)<- c("CD4", "CD8")
  ...
  ## Subset to untreated
  tenx_seurat_list<- lapply(tenx_seurat_list, function(seurat){subset(seurat, Treatment=="Untreated")})
  
  ## CD4 cell names have "_1" appended- fix
  tenx_seurat_list$CD4<- RenameCells(tenx_seurat_list$CD4, new.names= gsub("_1", "", colnames(tenx_seurat_list$CD4)))
  

## TCR
tcr<- read.csv("data/VDJ/multi/per_sample_outs/BT1857/vdj_t/filtered_contig_annotations.csv")
