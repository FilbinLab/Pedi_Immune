...
## broad annotation (myeloid, tcell) + detailed (cd4, cd8) based on marker gene expression
NO LONGER IN USE
...
analysis_dir<- "analysis/manual_broad.detailed_annot/"
figure_dir<- "figures/manual_broad.detailed_annot/"
data_dir<- "data/"
...
seurat_obj<- readRDS(paste0(data_dir, "immune_seurat_obj.Rds"))
```
...
ggsave(paste0(figure_dir, "UMAP_CD3_ByTcell.Myeloid.AC_markers.png"), width=12, height=4)

VlnPlot(seurat_cd3, group.by = "seurat_clusters", features = c("CD3G", "CD3E", "CSF1R", "GFAP"), ncol=4)
ggsave(paste0(figure_dir, "Vln_CD3_ByTcell.Myeloid.AC_markers.png"), width=12, height=4)

DimPlot(seurat_cd3, label=TRUE)+NoLegend()
ggsave(paste0(figure_dir, "UMAP_CD3_seurat.clusters.png"), width=6, height=6)
...
names(annots)<- c("Tcell", "Tcell", "Tcell", "Tcell", "Myeloid", "Tcell", "Tcell", "Tcell")
...
ggsave(paste0(figure_dir, "UMAP_CD3_broadAnnot.png"), width=6, height=6)
...
ggsave(paste0(figure_dir, "UMAP_Tcells_byCD4.CD8.png"), width=15, height=5)

VlnPlot(tcell_seurat, features=c("CD3G","CD3E", "CD4", "CD8A"), ncol = 4)
ggsave(paste0(figure_dir, "Vln_Tcells_byCD4.CD8.png"), width=12, height=4)

DimPlot(tcell_seurat, label=TRUE)+NoLegend()
ggsave(paste0(figure_dir, "UMAP_Tcells_bySeuratClusters.png"), width=6, height=5)

## Clusters 6,8 show no expression of either cd4/cd8, but mostly express cd3?
unclearClusters_seurat<- subset(tcell_seurat, seurat_clusters %in% c(6,8))
...
ggsave(paste0(figure_dir, "UMAP_Tcells_unclearRemoved_byCD4.CD8.png"), width=15, height=5)

VlnPlot(tcell.cd4.cd8_seurat, features=c("CD3G","CD3E", "CD4", "CD8A"), ncol = 4)
ggsave(paste0(figure_dir, "Vln_Tcells_unclearRemoved_byCD4.CD8.png"), width=12, height=4)

DimPlot(tcell.cd4.cd8_seurat, label=TRUE)+NoLegend()
ggsave(paste0(figure_dir, "UMAP_Tcells_unclearRemoved_bySeuratClusters.png"), width=6, height=5)
...
DimPlot(tcell.cd4.cd8_seurat, group.by = "detailed_annot")+
  scale_color_manual(values=c(CD4="navy", 
                              CD8="cadetblue3"))
ggsave(paste0(figure_dir, "UMAP_Tcell_unclearRemoved_byCD4.CD8Annot.png"), width=6, height=5)
...
## Plot myeloid/tcell annotated cells with rest of cells (astrocytes)- just for summary purposes
```{r}
data_dir<- "analysis/Preprocessing/"
seurat_obj<- readRDS(paste0(data_dir, "immune_seurat_obj.Rds"))
immune_seurat<- readRDS(paste0(analysis_dir, "seurat_cd3.Rds"))
