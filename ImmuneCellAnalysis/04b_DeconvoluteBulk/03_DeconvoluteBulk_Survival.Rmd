...
cbtn_meta<- read.csv(paste0(data_dir,"CBTN/clinical_metadata_withHistoneStatus.csv"))
  ...
  clinical_cbtn<- clinical_cbtn %>% dplyr::rename("submitter_id"="Patient.ID",
                                                  "time"="OS_MONTHS",
...
cb_res<- cb_cbtn[cb_cbtn$Mixture %in% cbtn_meta$Patient.ID[cbtn_meta$HistoneStatus=="WT"],]
clinical_use<- clinical_cbtn[clinical_cbtn$Patient.ID %in% cb_res$Mixture,]
```
...
myeloid_cellTypes<- colnames(cb_res)[!colnames(cb_res) %in% c(cd4_cellTypes, cd8_cellTypes, "Mixture", "Tumor", "P.value", "Correlation", "RMSE")]
...
ggsave(paste0(figure_dir_use, "CBTN_broadCellTypeProp.png"), width=6, height=3)
```
...
ggplot(res_cd8, aes(x=Sample, y=perCells, fill=Program))+
  ...
  theme(axis.text.x = element_blank())+
  ggtitle("CBTN Cibersort predicted proportions: CD8 programs")
ggsave(paste0(figure_dir_use, "CBTN_CD8_programProp.png"), width=6, height=3)
...
ggplot(res_cd4, aes(x=Sample, y=perCells, fill=Program))+
  ...
  theme(axis.text.x = element_blank())+
  ggtitle("CBTN Cibersort predicted proportions: CD4 programs")
ggsave(paste0(figure_dir_use, "CBTN_CD4_programProp.png"), width=6, height=3)
...
ggplot(res_myeloid, aes(x=Sample, y=perCells, fill=Program))+
  ...
  theme(axis.text.x = element_blank())+
  ggtitle("CBTN Cibersort predicted proportions: Myeloid programs")
ggsave(paste0(figure_dir_use, "CBTN_Myeloid_programProp.png"), width=6, height=3)
...
```{r}
## CBTN
if(grepl("CBTN", cohort)){
  clinical<- clinical_use[,c("Patient.ID", "OS_MONTHS", "OS_STATUS")]
  colnames(clinical)<-c("Sample", "time", "censored")
  clinical$censored<-as.logical(as.numeric(gsub(":DECEASED", "", gsub(":LIVING", "", clinical$censored))))
}
...
clinical_quant<- merge(cd8_quant_split, clinical, by="Sample")
...
## Merge clinical and quantile
clinical_quant<- merge(cd4_quant_split, clinical, by="Sample")
...
## Merge clinical and quantile
clinical_quant<- merge(myeloid_quant_split, clinical, by="Sample")
...
## Merge clinical and quantile
clinical_quant<- merge(broad_quant_split, clinical, by="Sample")
