...
final_PA_gene_list<- final_PA_gene_df %>% group_by(cluster) %>% top_n(n=100, wt=avg_log2FC) %>% as.data.frame()
...
## Heatmap of top marker genes
  ...
  ggsave(paste0(figure_dir, "Heatmap_topMarkers_MergedRegeneratedPedAdultMarkers_centeredBy", center_method, ".png"), width=15, height=10)
```

## For visualization purposes: harmony integration --> plot UMAP of sample, program
```{r}
merged_seurat<- readRDS(paste0("analysis/Merge_AssessIntegration/",cohort,"/adult_withMUV.FALSE/myeloid_seurat_Ped.AdultGBM.IDHMut_merged.Rds" ))
...
ggsave(paste0(figure_dir, "UMAP_harmony_byAge.png"), width=7, height=7)

DimPlot(merged_seurat, group.by="MaxProgram_PA_mergedGenesets")+
  scale_color_manual(values=colors_use)+
  ggtitle("")+
  theme(legend.position = "bottom")
ggsave(paste0(figure_dir, "UMAP_harmony_byMergedRegeneratedPrograms.png"), width=7, height=7.3)
...
FeaturePlot(merged_seurat, features="MM_score")+
  ...
  theme(legend.position = "bottom")
ggsave(paste0(figure_dir, "UMAP_harmony_byMacrophage.MicrogliaScore.png"), width=7, height=7.3)
