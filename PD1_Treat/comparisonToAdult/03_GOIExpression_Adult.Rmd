...
```{r}
tenx_seurat<- list(CD4=pd1_cd4, CD8=pd1_cd8)
...
all_plots<- lapply(names(tenx_seurat), function(x){
  ...
  ## Plot
  myHeatmap(pb_center, GOI=GOI, min.value = -3, max.value = 3, facetWrap = FALSE,
...
cowplot::plot_grid(plotlist=all_plots, ncol=2)
ggsave(paste0(figure_dir, "Heatmap_GOI_Tcells_byTreatment.png"), width=10, height=7)
...
all_plots<- lapply(names(tenx_seurat), function(x){
  ...
  myHeatmap(pb_center, GOI=GOI, min.value = -3, max.value = 3, facetWrap = FALSE,
            ...
            orderSubtypes= c("GBM.new", "GBM.rec", "GBM.PD1")) +
    scale_fill_gradient2(high="red", low="blue", mid="white", name="Centered log2(TPM)")+
    ggtitle(x)
})
cowplot::plot_grid(plotlist=all_plots, ncol=2)
ggsave(paste0(figure_dir, "Heatmap_GOI_Tcells_byTreatment_Primary.Recurrent.png"), width=10, height=7)
```
