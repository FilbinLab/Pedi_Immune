...
## Set GOI to plot, sample order (based on treatment, sequencing)
```{r}
...
## Remove effector memory- no untreated GBM effector memory
tmp<- subset(tmp, Program_projected != "Effector\nMemory")

## Boxplot
goi_plot<- c("PDCD1", "GNLY", "TIGIT", "CTLA4", "GZMB", "SELPLG", "PTGER2", "KLRB1")
...
ggsave(paste0(figure_dir, "Boxplot_AllGOI_MeanExprBySample_PD1vUntreated.png"), width=10, height=4*length(goi_plot))
...
VlnPlot(tmp, group.by = "Program_projected",stack=TRUE , flip = TRUE,
  ...
  scale_fill_manual(values= c(Untreated="purple", PD1="orange"))
ggsave(paste0(figure_dir, "Vln_GOI_byTreatment.png"), width=7, height=8)
```
