...
pd1_seurat<- readRDS("analysis/preprocess/HarmonyInt/tcell.myeloid_seurat.Rds")
...
## Based on no expression of MS4A1- seems to not be B cells. Markers: CLEC4C, GZMB. Seems to be plasmacytoid dendritic cells. Removed either way, but show markers to confirm
myeloid_pd1$detailed_annot_projected_manualPDC<- gsub("BCell", "Plasmacytoid_DC", myeloid_pd1$detailed_annot_projected)
cowplot::plot_grid(plotlist=lapply(c( "CSF1R",  "CD207", "CLEC4C"), function(x){
  VlnPlot(myeloid_pd1, features=x, group.by = "detailed_annot_projected_manualPDC")+ NoLegend()+
    scale_fill_manual(values=c(immune_colors[names(immune_colors) %in% myeloid_pd1$detailed_annot_projected],
                               Plasmacytoid_DC="purple"))+
    xlab("")
}), ncol=3)
ggsave(paste0(figure_dir, "Vln_myeloid_detailedMarkers_projectedFromFull_manualPDCannot.png"), width=9, height=4)

DimPlot(myeloid_pd1, group.by = "detailed_annot_projected_manualPDC")+
  scale_color_manual(values=c(immune_colors[names(immune_colors) %in% myeloid_pd1$detailed_annot_projected],
                               Plasmacytoid_DC="purple"))
ggsave(paste0(figure_dir, "UMAP_myeloid_detailedAnnot_projectedFromFull_manualPDCannot.png"), width=6, height=6)
