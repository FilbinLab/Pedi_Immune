...
analysis_dir_tmp<- paste0(working_dir, "/analysis/broad_annotation/")
figure_dir_tmp<- paste0(working_dir, "/figures/broad_annotation/")
...
```{r}
integration<- "HarmonyInt/" ## "_harmony" or ""
figure_dir<- paste0(figure_dir_tmp, integration, "/"); if(!dir.exists(figure_dir)){dir.create(figure_dir)}
analysis_dir<- paste0(analysis_dir_tmp, integration, "/");if(!dir.exists(analysis_dir)){dir.create(analysis_dir)}

immune_seurat<- readRDS(paste0("analysis/preprocess/",integration, "tcell.myeloid_seurat.Rds"))
```
...
VlnPlot(immune_seurat, features=c("CD4", "CD8A", "CD3G","CD2", "CD247", "CD14", "CSF1R"), sort=TRUE, ncol= 1)
...
## Two small clusters seem to not express CD3 or CSF1R- DC? Bcell? AC? Endothelial?
```{r}
FeaturePlot(immune_seurat, features=c("CSF1R", "CD3G", "MS4A1", "CD207", "GFAP"))
ggsave(paste0(figure_dir, "UMAP_Tcell.Myeloid.DC.Bc.AC_markers.png"), width=10, height=15)

## Based on this, seems to be none? Increase resolution slightly to split these clusters away from others, look for DEGs
immune_seurat<- FindClusters(immune_seurat, resolution = 2)
DimPlot(immune_seurat, label=TRUE)+NoLegend()
ggsave(paste0(figure_dir, "UMAP_seuratClusters_res2.png"), width=6, height=6)

## Merge all Tcells/myeloid- too much heterogeneity here, just interested in markers for these 2 small clusters
immune_seurat$broad_annot_tmp<- ifelse(immune_seurat$seurat_clusters %in% c(7,3,9,11), "Myeloid",
                                       ifelse(immune_seurat$seurat_clusters %in% c(4,5,6,1,8,12,2,0), "Tcell",
                                              as.character(immune_seurat$seurat_clusters)))

## Find markers- markers for clusters 11 and 14?
immune_seurat<- SetIdent(immune_seurat, value=immune_seurat$broad_annot_tmp)
all_markers<- FindAllMarkers(immune_seurat, min.pct = 0.2)
...
DoHeatmap(immune_seurat, features=top_markers$gene)+scale_fill_gradient2(low="blue", mid="white", high="red")
ggsave(paste0(figure_dir, "Heatmap_UnclearClusters10.13ID.png"), width=6, height=6)

VlnPlot(immune_seurat, features=c("CSF1R", "CD3G"))

## cluster 10 seems to be Tcells (maybe low qual, based on heat shock protein expression?)
## cluster 13 seems to be myeloid or Bcells- shows expression of Bcell, DC, macrophage, and microglia
## For now: removing both of these clusters (only 33 cells)
clusters_keep<- unique(immune_seurat$seurat_clusters)[!unique(immune_seurat$seurat_clusters) %in% c(10,13)]
immune_seurat<- subset(immune_seurat, seurat_clusters %in% clusters_keep)

immune_seurat$broad_annot_tmp<- NULL
```

## Assign each cluster to myeloid/Tcell
```{r}
#immune_seurat<- FindClusters(immune_seurat, resolution = 2)
VlnPlot(immune_seurat, features=c("CSF1R", "CD3G"), group.by = "seurat_clusters")

immune_seurat$broad_annot<- ifelse(immune_seurat$seurat_clusters %in% c(0,1,2,4,5,6,8,12), "Tcell",
                                   ifelse(immune_seurat$seurat_clusters %in% c(3,7,9,11), "Myeloid", "issue"))
```
...
This analysis pending- not sure if needed
```{r}
## Read in cell assignments for rank6
cell_assignemnts<-readRDS(paste0("../../NMF/All_Tcells/analysis/", integration, "/", cohort, "/cell_assignment_rank6.Rds"))
...
## no integration results: one program doesn't express tcell markers highly, but does express myeloid
  ## NMF2 in pedOnly, NMF5 in pedOnly_nomuv63, NMF3 in pedOnly_nomuv63.withmuv91
  ## This cluster corresponds mostly to clusters 1/2 in full myeloid/tcell seurat object
  ## Overall, clusters 1/2 express Tcell markers, but this subset apparently does not
...
unclearNMF<- "2" ## 2 for pedOnly noInt, 5 for pedOnly_nomuv63 noInt
...
## NMF2/5/3 has very low (but not 0) CD3G expression. Are cells expressing both, or are there some Tcells/some myeloid in NMF2?
...
## 4 cell subsets for NMF2/5/3: NEITHER expressed (34/34/7) / MYELOID expressed (40/29/3) / TCELL expressed (42/50/123) / BOTH expressed (12/14/21)
...
tcell_cells<- rownames(marker_expr)[marker_expr$MyeloidExpr=="NotMyeloid" & marker_expr$TcellExpr == "Tcell"]
  ...
  tcell_seurat$re_annotation<- ifelse(colnames(tcell_seurat) %in% tcell_cells, paste0("TcellNMF", unclearNMF),
                                      "OtherTcellNMF") 
...
p2<-DimPlot(tcell_seurat, group.by="re_annotation")+
  scale_color_manual(values=c(OtherTcellNMF="grey", TcellNMF2="red"))+
...
## fix broad annotation- pending, not needed now (no Tcell NMF)
