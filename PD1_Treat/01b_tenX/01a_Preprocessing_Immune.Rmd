...
## Reduce resolution (+ merge) to identify markers for major clusters
seurat_obj<-FindClusters(seurat_obj, resolution = 0.3)
DimPlot(seurat_obj, label=TRUE)
seurat_obj$seurat_clusters<- gsub(1,0,  seurat_obj$seurat_clusters)
...
## Assign cluster AC/tumor clusters
seurat_obj$immune.other_annot<- ifelse(seurat_obj$seurat_clusters==8, "AC", ifelse(seurat_obj$seurat_clusters==5, "Tumor", "Immune"))
FeaturePlot(seurat_obj, features=c("CD3G", "CSF1R", "SPARCL1", "SOX3")) + DimPlot(seurat_obj, group.by = "immune.other_annot")
...
cm1<- apply(cm1, 2, function(x){x/sum(x) * 1e06})
