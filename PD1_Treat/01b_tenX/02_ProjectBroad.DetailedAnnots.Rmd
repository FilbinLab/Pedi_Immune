...
## Very few Bcells and no dendritic cells predicted based on projection. Seems to be DC based on IL3RA expression/outlier cluster?
...
ggsave(paste0(figure_dir, "UMAP_Myeloid_detailedMarkers.png"), width=6, height=8)
  ...
  ## subcluster potential bcells/dc
  c12<- subset(myeloid_pd1, seurat_clusters==12)
  c12<- RunFullSeurat(c12@assays$RNA@counts, RunHarmony = TRUE, samples=c12$sample, dims=89, pca_dims = 89)
  c12<- FindClusters(c12, resolution = 1)
  FeaturePlot(c12, features=c("CSF1R", "IL3RA","CD207", "CD19")) + DimPlot(c12, label=TRUE)
  c12$detailed_annot<- ifelse(c12$seurat_clusters == 0, "Myeloid", ifelse(c12$seurat_clusters==1, "DC", "BCell"))
  FeaturePlot(c12, features=c("CSF1R", "IL3RA","CD207", "CD19")) + DimPlot(c12,group.by = "detailed_annot", label=TRUE)
  ggsave(paste0(figure_dir, "Myeloid_recluster_Bcell.DC.png"), width=6, height=6)
  
  ## Add back to full seurat
  myeloid_pd1$detailed_annot_projected_manual<- ifelse(myeloid_pd1$seurat_clusters == 12,
                                                       as.character(plyr::mapvalues(colnames(myeloid_pd1),
                                                                                    colnames(c12),
                                                                                    c12$detailed_annot,
                                                                                    warn_missing = FALSE)),
                                                       myeloid_pd1$detailed_annot_projected)
...
DimPlot(myeloid_pd1, group.by="detailed_annot_projected_manual", pt.size = 1, label=TRUE, label.size = 6)+ NoLegend()+
  scale_color_manual(values=immune_colors[names(immune_colors) %in% myeloid_pd1$detailed_annot_projected_manual])
ggsave(paste0(figure_dir, "UMAP_myeloid_detailedAnnot_projectedFromFull.png"), width=6, height=6)
  
## Confirm high expression of myeloid markers by predicted detailed annotation
cowplot::plot_grid(plotlist=lapply(c( "CSF1R",  "CD207", "MS4A1", "IL3RA"), function(x){
  VlnPlot(myeloid_pd1, features=x, group.by = "detailed_annot_projected_manual")+ NoLegend()+
    scale_fill_manual(values=immune_colors[names(immune_colors) %in% myeloid_pd1$detailed_annot_projected_manual])+
    xlab("")
}), ncol=4)
