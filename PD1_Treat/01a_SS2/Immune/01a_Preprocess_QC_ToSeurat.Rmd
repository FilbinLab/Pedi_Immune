...
cm_new<- read.csv("data/BT1910new_cm_tpm.csv", row.names = "gene_id")
cm_old<- read.csv("data/BT1910_cm_tpm.csv", row.names="gene_id")
cm_new<- cm_new[rownames(cm_new) %in% rownames(cm_old),]
new_addGenes<- rownames(cm_old)[!rownames(cm_old) %in% rownames(cm_new)]
cm_new_addGenes<- as.data.frame(matrix(0, nrow=length(new_addGenes), ncol(cm_new)))
colnames(cm_new_addGenes)<- colnames(cm_new); rownames(cm_new_addGenes)<- new_addGenes
cm_new<- rbind(cm_new, cm_new_addGenes)
cm_new<- cm_new[rownames(cm_old),]
cm_new<- as.data.frame(apply(cm_new, 2, function(x){x/sum(x) * 1e06}))
colnames(cm_new)<- gsub("BT1910", "BT1910new", colnames(cm_new))

## read in count matrices
cm_files<- list.files("data/", pattern="tpm.csv")
cm_files<- cm_files[!grepl("new", cm_files)]
cm<-cm_new
...
metadata$patient<- ifelse(grepl("BT1910", metadata$sample), "BT1910", "BT1935")
```
...
for (i in sample) {
  ...
  print (paste0(sum(seurat_obj_split[[i]][["percent_mito"]] > .1), " of ", ncol(seurat_obj_split[[i]]),
...
## Change sample id to patient id- merging BT1910 old/new
## Only kept separate to 1) visualize QC stats 2) rename cells to make sure tracer info matches up
seurat_obj$sample<- ifelse(grepl("BT1910", seurat_obj$sample), "BT1910", seurat_obj$sample)
