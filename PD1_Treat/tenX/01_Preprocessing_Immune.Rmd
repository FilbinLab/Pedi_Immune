...
all_matrix<- lapply(samples, function(x){
  print(x)
  matrix_dir<- paste0(data_dir, x, "/Tumor")
...
sum(rownames(all_matrix$BT2062) != rownames(all_matrix$BT2080))
cm<- do.call("cbind", all_matrix)

## There are 91 genes that are duplicated (same for both samples)-sum counts
