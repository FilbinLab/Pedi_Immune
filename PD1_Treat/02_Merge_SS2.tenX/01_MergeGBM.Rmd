...
## BT1857 is in ss2 and tenx cohort- add to sample id to distinguish these samples
tenx_seurat_list<- lapply(tenx_seurat_list, function(x){
  x$sample<- ifelse(x$sample=="BT1857", paste0("BT1857_TenX"), x$sample)
  return(x)
  })

ss2_seurat_list<- lapply(ss2_seurat_list, function(x){
  x$sample<- ifelse(x$sample=="BT1857", paste0("BT1857_SS2"), x$sample)
  return(x)
  })
