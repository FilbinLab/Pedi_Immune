...
seurat_list<- lapply(seurat_list, function(x){
  x$clonal_group<- ifelse(x$Sequencing=="TenX", x$raw_clonotype_id, x$clonal_group)
  x$group_size<- ifelse(x$Sequencing=="TenX", x$clonotype_size, x$group_size)
  x$tcr_reconstructed<- ifelse(x$Sequencing=="TenX", ifelse(is.na(x$clonotype_size), "No", "Yes"), x$tcr_reconstructed)
  ...
  x$clonal_group<- ifelse(is.na(x$clonal_group), "NA", x$clonal_group)
  x$group_size<- ifelse(is.na(x$group_size), "NA", x$group_size)
  
  ## if tcr is reconstructed but group size =1 or NA: reassign as nonclonal
  x$clonal<- ifelse( (is.na(x$group_size) | x$group_size==1 | x$group_size=="NA") & x$tcr_reconstructed=="Yes", "nonclonal", x$clonal)
  ...
  x$group_size<- ifelse(x$tcr_reconstructed=="Yes" & x$group_size=="NA", 1, x$group_size)
  
  return(x)
})

## TEMPORARY MEASURE: BT1857 tenx does not have clonality info added. Add manually here- note that once untreated 10x analysis is rerun/organized, will be added at that time and this chunk can be deleted
bt1857TenX_dir<-"../..//ImmuneCellAnalysis/tenX/Immune/BT1857/data/VDJ/multi/per_sample_outs/BT1857/vdj_t/"
tcr<- read.csv(paste0(bt1857TenX_dir, "filtered_contig_annotations.csv"))
tcr$sample<- "BT1857_TenX"
tcr$barcode<- gsub("_", "-", tcr$barcode)
tcr$barcode<- paste0( "BT1857_TenX_",  tcr$barcode)
c_size<- as.data.frame(table(tcr$raw_clonotype_id))


  seurat_list<- lapply(seurat_list, function(seurat){
    seurat$clonal_group<- ifelse(seurat$sample=="BT1857_TenX",
                                 ifelse(paste0("BT1857_TenX_", colnames(seurat)) %in% tcr$barcode,
                                        as.character(plyr::mapvalues(paste0("BT1857_TenX_", colnames(seurat)),
                                                                     tcr$barcode, tcr$raw_clonotype_id, warn_missing = FALSE)),"NA"),
                                 seurat$clonal_group)
    
    seurat$group_size<- ifelse(seurat$sample=="BT1857_TenX",
                               as.character(plyr::mapvalues(seurat$clonal_group, c_size$Var1, c_size$Freq, warn_missing = FALSE)),
                               seurat$group_size)
    
    seurat$tcr_reconstructed<- ifelse(seurat$sample=="BT1857_TenX",
                                      ifelse(paste0("BT1857_TenX_", colnames(seurat)) %in% tcr$barcode,
                                             "Yes", "No"),
                                      seurat$tcr_reconstructed)
    seurat$clonal<- ifelse(seurat$sample=="BT1857_TenX",
                           ifelse(seurat$group_size>1 & seurat$group_size!="NA", "clonal",
                                  ifelse(seurat$group_size=="NA", "NA",
                                         ifelse(seurat$group_size==1, "nonclonal", "issue"))),
                           seurat$clonal)
    return(seurat)
...
all_plots<- lapply(names(ss2_list), function(celltype){
  ...
  clonal_meta<- seurat@meta.data[,c("clonal", "tcr_reconstructed", "clonal_group", "group_size", "Treatment", "sample")]
...
ggsave(paste0(figure_dir,"barchart_SS2_proportionClonallExpanded.png"), width=8, height=3)
...
all_plots<- lapply(names(tenx_list), function(celltype){
  ...
  ## subset to cells with tcr reconstructed
  clonal_meta<- clonal_meta[clonal_meta$tcr_reconstructed=="Yes",]
  clonal_meta$group_size<- as.numeric(clonal_meta$group_size)

  clonal_meta$Treatment<- factor(clonal_meta$Treatment, levels=c("Untreated", "PD1"))  
  ggplot(clonal_meta, aes(x=Treatment, y=log(group_size), fill=Treatment))+
...
all_plots<- lapply(names(tenx_list), function(celltype){
  ...
  clonal_meta<- clonal_meta %>% mutate(size_bin = cut(group_size, breaks=c(0, 2, 5, 10, 15, 20,50,75, max(clonal_meta$group_size))))
...
all_plots<- lapply(c("cd4", "cd8"), function(celltype){
  seurat<- tenx_list[[celltype]]
  seurat<- subset(seurat, group_size != "NA")
  seurat$group_size<- as.numeric(seurat$group_size)
  VlnPlot(seurat, group.by = "Treatment", features="group_size", sort=TRUE)+NoLegend()+
    scale_fill_manual(values=colors_treatment)+
...
all_plots<- lapply(c("cd4", "cd8"), function(celltype){
  ...
  seurat<- subset(seurat, group_size != "NA")
  seurat$group_size<- as.numeric(seurat$group_size)
  VlnPlot(seurat, group.by = "Program_projected", features="group_size", sort=TRUE)+NoLegend()+
...
all_plots<- lapply(c("cd4", "cd8"), function(celltype){
  ...
  seurat<- subset(seurat, group_size != "NA")
  seurat$group_size<- as.numeric(seurat$group_size)
  p1<- VlnPlot(subset(seurat, Treatment=="Untreated"), group.by = "Program_projected", features="group_size", sort=TRUE)+NoLegend()+
    scale_fill_manual(values=immune_colors)+
    ggtitle(paste0("Untreated Group size: ", celltype))+
    xlab("")+ ylab("clonotype group size")
  
  p2<- VlnPlot(subset(seurat, Treatment=="PD1"), group.by = "Program_projected", features="group_size", sort=TRUE)+NoLegend()+
    scale_fill_manual(values=immune_colors)+
...
all_plots<- lapply(c("cd4", "cd8"), function(celltype){
  ...
  seurat$group_size_norm<- seurat$group_size / seurat$NCells * 1000
  
  p1<- VlnPlot(subset(seurat, Treatment=="Untreated"), group.by = "Program_projected", features="group_size_norm", sort=TRUE)+NoLegend()+
    scale_fill_manual(values=immune_colors)+
    ggtitle(paste0("Untreated Group size: ", celltype))+
    xlab("")+ ylab("clonotype group size")
  
  p2<- VlnPlot(subset(seurat, Treatment=="PD1"), group.by = "Program_projected", features="group_size_norm", sort=TRUE)+NoLegend()+
    scale_fill_manual(values=immune_colors)+
    ggtitle(paste0("PD1 Group size: ", celltype))+
    xlab("")+ ylab("clonotype group size")
  p1+p2
...
all_plots<- lapply(seurat_list, function(seurat){
  ...
  meta$group_size<- as.numeric(meta$group_size)
